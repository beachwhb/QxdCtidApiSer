...
using QxdCtidApiSer.Migrations.SeedData;
...
{
    public abstract class QxdCtidApiSerTestBase : AbpIntegratedTestBase<QxdCtidApiSerTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected QxdCtidApiSerTestBase()
        {
        ...
        protected void UsingDbContext(Action<QxdCtidApiSerDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<QxdCtidApiSerDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<QxdCtidApiSerDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<QxdCtidApiSerDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<QxdCtidApiSerDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<QxdCtidApiSerDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<QxdCtidApiSerDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<QxdCtidApiSerDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<QxdCtidApiSerDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<QxdCtidApiSerDbContext, Task<T>> func)
        {
            T result;
