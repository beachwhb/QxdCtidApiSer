...
namespace QxdCtidApiSer.Web.Controllers
{
    public class CtidsController : QxdCtidApiSerControllerBase
    {
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult GetTwoAndFace(GetTwoAndFaceInput input)
        {
            HttpPostedFileBase file = Request.Files[0];


            var twoAndFaceInput = new GetTwoAndFaceInput();
            if (input.PhotoBuffer == null)
            {
                return new EmptyResult();
            }
            //var bytes = new byte[input.PhotoBuffer.ContentLength];
            //input.PhotoBuffer.InputStream.Read(bytes, 0, bytes.Length);
            //input.PhotoBuffer.InputStream.Seek(0, SeekOrigin.Begin);
            ...
            twoAndFaceInput.Name = input.Name;
            twoAndFaceInput.IdCardNo = input.IdCardNo;
            //twoAndFaceInput.PhotoBuffer = Convert.ToBase64String();
        ...
        [HttpPost]
        public ActionResult GetFourAndFace(GetFourAndFaceInput input)
        {
            ...
            var result = _ctidRecogAppService.GetFourAndFace(input);
